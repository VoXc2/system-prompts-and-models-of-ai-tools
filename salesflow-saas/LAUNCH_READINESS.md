...
**حالة التحقق الأخيرة (يجب إعادة تشغيلها قبل كل إصدار):** راجع أرقام الاختبارات الفعلية من `pytest` و`playwright` في CI. آخر تدقيق آلي (نفس المستودع): backend `73` ناجحة (`2` متخطاة اختيارياً لـ LangGraph)، Vitest `21`، Playwright `54`، بناء Next.js ناجح.
