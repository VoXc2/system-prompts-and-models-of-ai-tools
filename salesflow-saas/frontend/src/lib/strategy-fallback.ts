...
export const STRATEGY_SUMMARY_FALLBACK: StrategySummary = {
  product: "Dealix",
  blueprint_version: "5.0.0-sovereign",
  ...
  moat_pillars: [
    "Decision-native: recommendations are typed, evidence-backed, and policy-aware",
    "Execution-durable: commitments are resumable, idempotent, and observable",
    "Trust-enforced: authorization + policy + audit + verification ledger",
    "Arabic-first / Saudi-ready product design",
    "Enterprise-saleable surfaces for board and executive operations",
  ],
  competitive_moat: {
    decision_plane: "Responses API + Structured Outputs + LangGraph interrupts/HITL",
    execution_plane: "Temporal for durable business commitments",
    trust_plane: "OPA + OpenFGA + Vault + Keycloak",
    data_plane: "Postgres/pgvector + contracts + OTel",
    operating_plane: "GitHub rulesets/environments/OIDC/attestations",
  },
  auditable_targets: [
    ...
    { id: "traceability", label_ar: "التتبع التشغيلي", target: "OTel traces + correlation IDs عبر المسارات", unit: "observability" },
  ...
  design_principles: [
    ...
    { id: "arabic_saudi_first", title_ar: "عربي أولاً سعودي الجاهزية", summary: "المحتوى التنفيذي والأسباب بالعربية الاحترافية" },
    { id: "board_usable", title_ar: "جاهزية مجلس الإدارة", summary: "Evidence packs + approval center + risk heatmaps" },
  ],
  planes: [
    {
      id: "decision",
      name_ar: "Decision Plane",
      mission: "كشف الإشارات والتحليل والتوصية وتهيئة evidence packs مع HITL",
      stack: ["Responses API", "Structured Outputs", "Function Calling / MCP", "LangGraph checkpoints + interrupts"],
    },
    {
      id: "execution",
      name_ar: "Execution Plane",
      mission: "تنفيذ الالتزامات الطويلة متعددة الأنظمة بشكل deterministic",
      stack: ["LangGraph for cognition loops", "Temporal for durable commitments", "Idempotency + timeout + compensation"],
    },
    {
      id: "trust",
      name_ar: "Trust Plane",
    ...
    },
    {
      id: "data",
      name_ar: "Data Plane",
      mission: "مصدر حقيقة تشغيلي موحد مع تعاقدات بيانات ومراقبة جودة",
      stack: ["Postgres + pgvector", "Airbyte + Unstructured", "Great Expectations", "CloudEvents + JSON Schema + AsyncAPI", "OpenTelemetry"],
    },
    {
      id: "operating",
      name_ar: "Operating Plane",
      mission: "ضبط SDLC والإصدارات وحماية البيئات وإثبات provenance",
      stack: ["GitHub rulesets/protected branches", "Environments + deployment protections", "OIDC federation", "Artifact attestations", "External audit-log streaming"],
    },
  ],
  program_locks: {
    planes: 5,
  ...
  saudi_compliance_matrix: ["PDPL controls mapping", "ECC/NCA cyber controls checkpoints", "NIST AI RMF risk lifecycle", "OWASP LLM Top 10 mitigations"],
  phases: [
